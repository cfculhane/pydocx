...
def filter_children(element, tags):
    return [el for el in element.getchildren() if el.tag in tags]
...
def xml_remove_namespaces(xml_bytes):
    ...
    for child in el_iter(root):
        ...
        child.attrib = dict((k.split("}")[-1], v) for k, v in child.attrib.items())
...
def convert_dictionary_to_html_attributes(attributes):
    return " ".join('{k}="{v}"'.format(k=k, v=v) for k, v in sorted(attributes.items()))
